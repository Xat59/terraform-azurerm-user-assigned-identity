...
####
variable "identities" {
  description = "(Optional) A list of map that contains User Managed Identities."
  type        = set(object({
    name = string
    tags = map(string)
  }))
...
variable "roles" {
  description = "(Optional) A list of map that contains roles assignments."
  type        = list(object({
    name            = string
    scope           = string
...
#   type        = string
#   default     = "48031b6e-1bad-4ba0-bb2d-955609fcaf2e"
# }
